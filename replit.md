...
**AI Scan Tuning** (Pro/Admin only) uses GPT-5.1 to suggest parameter adjustments based on indicator metadata and funnel data, with guardrails for valid suggestions. A **Chart Rating System** allows users to rate scan results, feeding into personalized AI scan tuning. **Preset Scan Templates** offer curated scan configurations.
