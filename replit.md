...
**Scan Learning Loop** (Phases 1-4 complete): Every scan run creates a `scan_sessions` record (the "spine") linking scan config, result count, result symbols, and funnel data. Chart ratings include `sessionId` and full `indicatorSnapshot` (thoughtBreakdown with per-indicator criteriaResults and diagnostics). The tuning commit flow enforces a disciplined workflow: users experiment freely (Apply/Undo/More Suggestions), then "Review on Chart" rescans and opens the chart viewer for rating. Learning is ONLY recorded on explicit user commitment via "Save & Commit Tuning" — never during experimentation. Commit requires chart review (30% rating threshold). Discarding reverts all params and records the discard as negative signal. A navigation guard prevents losing uncommitted tuning changes. The `scan_tuning_history` table stores outcome (accepted/discarded), configBefore/configAfter, acceptedSuggestions/skippedSuggestions, symbol retention (retainedUpSymbols, droppedUpSymbols, droppedDownSymbols), ratingsCount, adminApproved status, plus enrichment fields: marketRegime (JSONB snapshot of weekly trend, daily basket, choppiness, SPY price from cached sentiment engine), universe (text), archetypeTags (text[] extracted from thought names via 18 regex patterns), tuningDirections (JSONB per-indicator tightened/loosened/mixed), and acceptanceRatio (float). **Phase 3-4 Hybrid Learning Context**: The AI tuning prompt uses a hybrid approach — all-time aggregated stats from `indicator_learning_summary` table (per-indicator paramStats, regime/universe/archetype bucketed performance, retention rates, avoid params) combined with last 30 raw sessions for recency. Context injection highlights CURRENT REGIME, CURRENT UNIVERSE, and matching ARCHETYPE performance so the AI adapts suggestions to current market conditions. The `indicator_learning_summary` table is incrementally upserted on admin-approved commits via rolling averages. **Admin Approval Gate**: Admin commits auto-approve and immediately feed learning summary; Pro user commits go to `pending_review` and appear in an admin review queue (Tuning Reviews tab) — approval triggers learning summary upsert, rejection blocks it.
